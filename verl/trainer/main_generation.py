...
def main(config):
    ...
    if os.path.exists(config.data.output_path):
        ...
        try:
            ...
            try:
                ...
                with open(config.data.output_path) as f:
        ...
        with open(config.data.path) as f:
            dataset = pd.read_json(f)
        ...
        for batch_idx in range(num_batch):
            ...
            inputs = tokenizer.apply_chat_template(repeated_chat_lst, add_generation_prompt=True, padding=True, truncation=True, max_length=config.rollout.prompt_length, return_tensors="pt", return_dict=True, tokenize=True)
            ...
            if real_batch_size % dp_size != 0:
                ...
                print(f"dp_size {dp_size} is not divisible by real_batch_size {real_batch_size}, add {dummy_data_size} dummy data")
    ...
    row_data = {"model_path": config.model.path, "dataset": dataset_name, "pass@1": pass_at_1, f"pass@{n_samples}": pass_at_n}
